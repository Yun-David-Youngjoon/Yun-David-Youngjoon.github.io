...
function undoCanvasAction() {
    if (canvasHistory.length > 0) {
        let lastAction = canvasHistory.pop();
        let previousImage = new Image();
        previousImage.src = lastAction;
...
const drop = (e) => {
    ...
    if (isDragging) {
        ...
        ctx.drawImage(drawImgEach, x, y, drawImgEach.width, drawImgEach.height);
        isDragging = false;
    }
}
...
canvas.addEventListener('dragover', (e) => e.preventDefault()); //Allow Dropping
canvas.addEventListener("drop", (e) => drop(e));
